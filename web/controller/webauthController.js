...
const db = require('./firebase'); // path to firebase.js
    ...
    if (validation.fails()) {
        ...
        auth.user_insert(req, addArray, async function (err, response) {
            if (err) {
                ...
                // generate auto push id
                const newUserRef = usersRef.push();

                await newUserRef.set({
                    id: newUserRef.key,
                    loginuserid: response.toString(), // from your SQL insert
                    nickname: req.body.first_name + ' ' + req.body.last_name,
                    photoUrl: req.body.photoUrl || '',
                    fcmToken: req.body.fcmToken || '',
                    status: 'online'
                });
                auth.updateFirebaseId(req, newUserRef.key, response.toString(), function (err, response) {
                            // if (err) {
                ...
                console.log("Done");
                            const temp_path = './admin/views/api_pages/welcome-salon-email-temp.ejs';
...
exports.resetPassword = async (req, res, next) => {
    ...
    try {
        ...
        if (user_data) {
            const temp_path = './web/views/auth_pages/reset-email-temp.ejs';
